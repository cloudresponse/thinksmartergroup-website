...
describe("Investment Interface", () => {
  describe("InvestmentCategory enum", () => {
    ...
    it("should have consistent enum structure", () => {
      expect(InvestmentCategory.EducationSkillsMedia).toBe(
        "Education, Skills and Media",
      );
      ...
      expect(InvestmentCategory.ProfessionalServices).toBe(
        "Professional Services",
      );
  ...
  describe("Investment interface structure", () => {
    it("should define required properties", () => {
      ...
      expect(Object.values(InvestmentCategory)).toContain(
        sampleInvestment.category,
      );
    });
  ...
  describe("Category usage patterns", () => {
    it("should support filtering by category", () => {
      ...
      const aiInvestments = investments.filter(
        (inv) => inv.category === InvestmentCategory.AI_SAAS,
      );
      const healthInvestments = investments.filter(
        (inv) => inv.category === InvestmentCategory.Healthcare,
      );
    ...
    it("should support category grouping", () => {
      ...
      const groupedByCategory = investments.reduce(
        (acc, inv) => {
          if (!acc[inv.category]) {
            acc[inv.category] = [];
          }
