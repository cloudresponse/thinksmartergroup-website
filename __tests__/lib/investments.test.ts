...
describe("Investments Data", () => {
  ...
  it("should have valid URLs for all investments", () => {
    investments.forEach((investment) => {
      try {
        ...
        fail(
          `Invalid URL for investment "${investment.title}": ${investment.href}`,
        );
      }
    });
  });
  ...
  it("should have investments with descriptions", () => {
    const investmentsWithDescriptions = investments.filter(
      (inv) => inv.description,
    );
    ...
    const descriptionPercentage =
      (investmentsWithDescriptions.length / investments.length) * 100;
