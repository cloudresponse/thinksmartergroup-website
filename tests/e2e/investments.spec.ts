...
test.describe('Investments Page', () => {
  ...
  test('should load the investments page successfully', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /our investments/i })).toBeVisible();
  });

  test('should display investments grid', async ({ page }) => {
    // Wait for investments to load
    await page.waitForSelector('[data-testid="investment-card"], .bg-black', { timeout: 10000 });
    
    // Check that investment cards are displayed
    const investmentCards = page.locator('[data-testid="investment-card"], .bg-black');
  ...
  test('should display category filter buttons', async ({ page }) => {
    ...
    await page.waitForSelector('button[class*="rounded-full"]', { timeout: 5000 });
  ...
  test('should filter investments by category', async ({ page }) => {
    ...
    const initialCards = page.locator('[data-testid="investment-card"], .bg-black');
    const initialCount = await initialCards.count();
    
    // Click on first category filter
    const firstCategoryButton = page.locator('button[class*="rounded-full"]').first();
    ...
    const filteredCards = page.locator('[data-testid="investment-card"], .bg-black');
  ...
  test('should clear filter when clicking selected category again', async ({ page }) => {
    // Wait for content to load
    await page.waitForSelector('[data-testid="investment-card"], .bg-black', { timeout: 10000 });
    await page.waitForSelector('button[class*="rounded-full"]', { timeout: 5000 });
    ...
    const finalCards = page.locator('[data-testid="investment-card"], .bg-black');
  ...
  test('should display investment card content correctly', async ({ page }) => {
    ...
    const firstCard = page.locator('[data-testid="investment-card"], .bg-black').first();
  ...
  test('should handle navigation from investment cards', async ({ page }) => {
    ...
    const firstCardLink = page.locator('[data-testid="investment-card"], .bg-black a').first();
  ...
  test('should display page description', async ({ page }) => {
    ...
    await expect(page.getByText(/we have made a range of investments/i)).toBeVisible();
    await expect(page.getByText(/we invest in the following sectors/i)).toBeVisible();
  });
